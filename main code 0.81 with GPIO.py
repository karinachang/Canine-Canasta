...
# defines the GPIO pins for the buttons
# in order W-A-S-D-select
buttons = [ 12, 20, 24, 16, 26 ]

# use the Broadcom pinmode
# idk if this is necessary but we used it in the Simon game
# so i am putting it in here
...
def runGame():
    ...
    while(gameStatus == "InProgress"):
        ...
        # helps with button debouncing
        pressed = False

        # as long as nothing is pressed
        while (not pressed):
            # check each button
            for i in range(len(buttons)):
                ...
                while (GPIO.input(buttons[i]) == True):
                    ...
                    val = i
                    # note that a button has been pressed
                    pressed = True
        ...
        # i left this function in so we can still manually input in the move values
        # i would like to remove it once its all done
        move = input("Move")
        ...
        # if the input is right
            ...
            try:
                ...
                pass
        # not sure what input this refers to as, is it select?
        # i left in the move input
        elif(move == " "):
            if(picked[0] == None):
                selectRender(selection)
